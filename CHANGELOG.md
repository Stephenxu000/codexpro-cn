...
- Refined controlled full Bash confirmation around side effects instead of raw shell characters: quoted regex operators, read-only pipelines, and `/dev/null` diagnostic redirection run without confirmation, while destructive/system/network/eval/command-substitution/file-output paths still require explicit confirmation.
- Added an optional `stable` tool-discovery surface: common development tools stay directly typed while low-frequency enabled capabilities remain callable through the stable `codexpro(action=...)` wrapper. Stable mode now also keeps hidden enabled tools callable for stale connector snapshots while filtering them out of `tools/list`, so cached direct calls such as `load_skill` or `inspect_workspace` no longer fail just because the discovery surface was narrowed. `server_config`, `/healthz`, and the wrapper expose schema/surface metadata and a capability fingerprint for stale-connector diagnosis; `expanded` remains the compatibility default.
- Aligned `reload_service` with the connector schema by allowing CodexPro self-reload through a fixed, detached ServerAdmin helper. The helper is scheduled only from an allowlisted `codexpro` action; ngrok/dashboard keep their existing fixed LaunchAgent kickstart path.
- Prefer the official standalone Codex CLI at `~/.local/bin/codex` for controlled local agent runs, while retaining Homebrew and legacy ChatGPT app-bundle paths as compatibility fallbacks so ChatGPT app updates no longer break `agent_run`.
